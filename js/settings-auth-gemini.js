    ...
    modal.innerHTML = `
        <div class="modal-content">
            <div class="modal-header">
                <h3 class="modal-title">Gemini CLI 认证</h3>
                <button class="modal-close" id="gemini-project-modal-close">&times;</button>
            </div>
            <div class="modal-body">
                <div class="codex-auth-content">
                    <p>请输入 Google Cloud 项目 ID（可选）：</p>
                    <div class="form-group">
                        <input type="text" id="gemini-project-id-input" class="form-input" placeholder="请输入项目 ID（可选）">
                        <small class="form-help">如不输入项目 ID，将使用默认项目</small>
                    </div>
                    <div class="auth-actions">
                        <button type="button" id="gemini-project-confirm-btn" class="btn-primary">确认</button>
                        <button type="button" id="gemini-project-cancel-btn" class="btn-cancel">取消</button>
                    </div>
                </div>
            </div>
        </div>`;
...
async function confirmGeminiProjectId() {
    try {
        ...
        showError('启动 Gemini CLI 认证流程失败：' + error.message);
    }
}
...
async function startGeminiAuthFlow() {
    try {
        ...
        showError('启动 Gemini CLI 认证流程失败：' + msg);
...
async function handleGeminiCallback(req, res) {
    try {
        ...
        res.end();
        setTimeout(async () => { await stopGeminiLocalServer(); showSuccessMessage('Gemini CLI 认证完成！'); }, 1000);
    } catch (error) {
            ...
            <div class="modal-body">
                <div class="codex-auth-content">
                    ...
                    <div class="auth-url-container">
                        ...
                        <button type="button" id="copy-gemini-url-btn" class="copy-btn">复制链接</button>
                    </div>
                    ...
                    <div class="auth-actions">
                        <button type="button" id="open-gemini-url-btn" class="btn-primary">打开链接</button>
                        <button type="button" id="cancel-gemini-btn" class="btn-cancel">取消</button>
                    </div>
                </div>
            </div>
        </div>`;
...
async function copyGeminiUrl() {
    try { const urlInput = document.getElementById('gemini-auth-url-input'); await navigator.clipboard.writeText(urlInput.value); showSuccessMessage('链接已复制到剪贴板'); }
    ...
    try {
        ...
        await pollGeminiAuthStatus(
            ...
            () => {
                ...
                showSuccessMessage('Gemini CLI 认证完成！');
                cancelGeminiAuth();
            ...
            (error) => {
                ...
                showError('Gemini CLI 认证失败：' + error);
                cancelGeminiAuth();
            }
        );
    } catch (error) {
        console.error('Gemini CLI Authentication polling error:', error);
        showError('Gemini CLI 认证过程中出错：' + error.message);
        cancelGeminiAuth();
    }
}
